...
def test_memos_api_exploration():
    ...
    try:
        ...
        print(
            "Available in MOS:", [attr for attr in dir(MOS) if not attr.startswith("_")]
        )

        # Try to understand GeneralMemCube structure
        print(
            "Available in GeneralMemCube:",
            [attr for attr in dir(GeneralMemCube) if not attr.startswith("_")],
        )
