...
def memory_entry_strategy(draw):
    ...
    return MemoryEntry(
        ...
        content=draw(st.text(min_size=1, max_size=1000, alphabet=st.characters(blacklist_characters='\x00', blacklist_categories=('Cs',)))),
...
class TestMemoryContextProperties:
    ...
    @given(st.lists(st.sampled_from(list(MemoryType)), min_size=0, max_size=len(MemoryType), unique=True))
    ...
    def test_memory_statistics_consistency(self, entries):
        ...
        async def test_stats():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                unique_ids = len(set(entry.id for entry in entries))
...
class TestMemoryConfigProperties:
    ...
    def test_memory_config_limits(self, max_memories: int):
        ...
        config = MemoryConfig(
            ...
            max_memories_per_agent=max_memories // 2
        )
    ...
    def test_memory_config_thresholds(self, threshold: float):
        ...
        config = MemoryConfig(
            default_importance_threshold=threshold,
