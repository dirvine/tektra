...
def memory_context_strategy(draw):
    ...
    return MemoryContext(
        ...
        time_window_hours=draw(
            st.one_of(
                st.none(), st.integers(min_value=1, max_value=8760)  # Up to 1 year
            )
        ),
        memory_types=draw(
            st.lists(
...
class TestMemoryContextProperties:
    ...
    def test_memory_context_serialization_roundtrip(self, context: MemoryContext):
        ...
        assert all(
            mt in [t.value for t in MemoryType] for mt in context_dict["memory_types"]
        )
...
class TestMemoryManagerProperties:
    ...
    def test_search_limit_property(self, entries: list[MemoryEntry], limit: int):
    ...
    @settings(deadline=20000)
    def test_importance_filtering_property(
        self, entries: list[MemoryEntry], min_importance: float
    ):
...
class TestMemorySystemInvariants:
    ...
    def test_statistics_consistency(self, entries: list[MemoryEntry]):
        ...
        async def test_stats():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if entries:
                    sum(entry.importance for entry in entries) / len(
                        entries
                    )
    ...
    @settings(deadline=25000)
    def test_concurrent_access_safety(self, entries: list[MemoryEntry]):
        ...
        async def test_concurrent():
            with tempfile.TemporaryDirectory() as temp_dir:
                config = MemoryConfig(
                    storage_path=temp_dir,
                    database_name="concurrent_test.db",
                    max_memories_per_user=1000,
                    use_memos=False,
                    enable_semantic_search=False,
                )
    ...
    @settings(deadline=20000)
    def test_cleanup_preserves_invariants(self, entries: list[MemoryEntry]):
        ...
        async def test_cleanup():
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Perform cleanup
                await memory_manager.cleanup_old_memories(days=1)
