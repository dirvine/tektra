...
from tektra.memory import MemoryConfig, MemoryContext, MemoryType, TektraMemoryManager
...
async def test_memory_manager_initialization():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config = MemoryConfig(
                storage_path=temp_dir, use_memos=False  # Skip MemOS for now
            )
...
async def test_memory_operations():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            config = MemoryConfig(storage_path=temp_dir, use_memos=False)
            ...
            search_context = MemoryContext(
                user_id=user_id, query="help", max_results=10
            )
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            agents = [
                (
                    "github_monitor",
                    "Monitor GitHub repositories for issues and pull requests",
                ),
                (
                    "stock_analyzer",
                    "Analyze stock prices and send alerts for significant changes",
                ),
                (
                    "file_organizer",
                    "Organize files by type and maintain folder structure",
                ),
            ]
            ...
            for agent_id, _ in agents:
                ...
                assert (
                    len(agent_context) == 1
                ), f"Expected 1 context entry for {agent_id}, got {len(agent_context)}"
                assert (
                    agent_context[0].agent_id == agent_id
                ), f"Context doesn't belong to {agent_id}"

                print(f"✅ Agent {agent_id} memory isolation verified")

            # Test cross-agent search
            all_agents_context = MemoryContext(
                memory_types=[MemoryType.AGENT_CONTEXT], max_results=10
            )

            all_results = await memory_manager.search_memories(all_agents_context)
            print(
                f"✅ Cross-agent search found {len(all_results.entries)} context entries"
            )
            ...
            assert (
                agent_ids_found == expected_agents
            ), f"Missing agents: {expected_agents - agent_ids_found}"
