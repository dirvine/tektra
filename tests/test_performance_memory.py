...
class TestMemoryPerformance:
    ...
    def create_test_entries(
        self, count: int, base_user_id: str = "perf_user"
    ) -> list[MemoryEntry]:
    ...
    async def test_memory_insertion_performance(self, memory_manager):
        ...
        assert (
            avg_individual_time < 50
        ), f"Individual insertion too slow: {avg_individual_time:.2f}ms"

        avg_batch_time = batch_time / len(batch_entries)
        assert (
            avg_batch_time < 20
        ), f"Batch insertion too slow: {avg_batch_time:.2f}ms per entry"
    ...
    async def test_memory_retrieval_performance(self, memory_manager):
        ...
        assert (
            avg_retrieval_time < 10
        ), f"Memory retrieval too slow: {avg_retrieval_time:.2f}ms"
    ...
    async def test_memory_search_performance(self, memory_manager):
        ...
        for term in search_terms:
            search_context = MemoryContext(
                query=term, max_results=10, user_id="search_user_0"
            )
            _, exec_time = await self.async_benchmark_function(
    ...
    async def test_concurrent_operations_performance(self, memory_manager):
        ...
        tasks = [insert_batch(i * 100, 100) for i in range(5)]
        ...
        ), f"Concurrent operations too slow: {avg_concurrent_time:.2f}ms per entry"

        print(f"Concurrent insertions: {avg_concurrent_time:.2f}ms avg per entry")
        print(
            f"Total concurrent time: {concurrent_time:.2f}ms for {total_inserted} entries"
        )
    ...
    async def test_large_dataset_performance(self, memory_manager):
        ...
        search_context = MemoryContext(
            query="performance test", max_results=50, user_id="large_user_0"
        )
        ...
        assert (
            avg_insertion_time < 25
        ), f"Large dataset insertion too slow: {avg_insertion_time:.2f}ms per entry"
    ...
    async def test_memory_usage_scaling(self, memory_manager):
        ...
        for size in dataset_sizes:
            ...
            # Test search performance
            search_context = MemoryContext(
                query="test entry", max_results=10, user_id=f"scale_user_{size}_0"
            )
        ...
        for i, metrics in enumerate(performance_metrics):
            ...
            if i > 0:
                ...
                time_ratio = (
                    metrics["avg_insertion_time"] / prev_metrics["avg_insertion_time"]
                )

                # Time should not increase more than linearly (allowing for improvements due to caching)
                assert (
                    time_ratio < size_ratio * 2.0
                ), f"Performance degrades too quickly with size: {time_ratio:.2f}x for {size_ratio:.2f}x size"
...
class TestMemoryBenchmarks:
    ...
    def benchmark_memory_operations(
        self, name: str, operation_func, iterations: int = 100
    ):
        ...
        return {"average": avg_time, "median": median_time, "p95": p95_time}
    ...
    def test_memory_entry_creation_benchmark(self):
        ...
        results = self.benchmark_memory_operations(
            "Memory Entry Creation", create_entry, 1000
        )

        # Should be very fast
        assert (
            results["average"] < 1.0
        ), f"Entry creation too slow: {results['average']:.2f}ms"
    ...
    def test_memory_entry_serialization_benchmark(self):
        ...
        serialize_results = self.benchmark_memory_operations(
            "Serialization", serialize_entry, 1000
        )
        deserialize_results = self.benchmark_memory_operations(
            "Deserialization", deserialize_entry, 1000
        )

        # Should be fast
        assert (
            serialize_results["average"] < 2.0
        ), f"Serialization too slow: {serialize_results['average']:.2f}ms"
        assert (
            deserialize_results["average"] < 3.0
        ), f"Deserialization too slow: {deserialize_results['average']:.2f}ms"
    ...
    async def test_database_operations_benchmark(self, memory_manager):
        ...
        async def search_entries():
            context = MemoryContext(
                query="benchmark entry", max_results=10, user_id="db_user"
            )
