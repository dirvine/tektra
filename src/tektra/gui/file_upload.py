...
class FileUploadPanel:
    ...
    def __init__(
        ...
        on_file_processed: Callable | None = None,
    ...
    def _build_ui(self) -> toga.Box:
        ...
        self.container = toga.Box(
            style=Pack(
                direction=COLUMN, background_color="#f8f9fa", margin=10, padding=15
            )
        )
        ...
        header = toga.Label(
            "📎 File Upload",
            style=Pack(
                font_size=16, font_weight="bold", margin_bottom=10, color="#2c3e50"
            ),
        )
        ...
        self.upload_button = toga.Button(
            ...
            style=Pack(
                background_color="#007bff", color="#ffffff", margin_right=10, width=120
            ),
        )
        upload_row.add(self.upload_button)

        self.status_label = toga.Label(
            "No files selected", style=Pack(font_size=12, color="#6c757d", flex=1)
        )
        ...
        # File list container
        self.file_list_container = toga.Box(
            style=Pack(
                direction=COLUMN, margin_top=5, background_color="#ffffff", padding=10
            )
        )

        # Scroll container for file list
        self.file_scroll = toga.ScrollContainer(
            content=self.file_list_container, style=Pack(height=150, margin_bottom=10)
        )
        ...
        self.process_button = toga.Button(
            ...
            style=Pack(background_color="#28a745", color="#ffffff", width=120),
            enabled=False,
        )
        ...
        info_label = toga.Label(
            ...
            style=Pack(
                font_size=10, color="#6c757d", margin_top=5, font_style="italic"
            ),
        )
    ...
    async def show_file_dialog(self, dialog) -> list[Path] | None:
        """
    ...
    def _create_file_row(self, file_path: Path, index: int) -> toga.Box:
        ...
        name_label = toga.Label(
            file_path.name,
            style=Pack(font_weight="bold", font_size=12, margin_bottom=2),
        )
        ...
        try:
            ...
            logger.debug(f"Failed to get file stats for {file_path.name}: {e}")
            info_text = f"{file_path.suffix.upper()[1:]} file"

        info_label = toga.Label(info_text, style=Pack(font_size=10, color="#6c757d"))
        file_info.add(info_label)

        # Add icon and info to row
        icon_label = toga.Label(file_icon, style=Pack(font_size=16, margin_right=10))
        row.add(icon_label)
        row.add(file_info)
        ...
        remove_btn = toga.Button(
            "✕",
            on_press=lambda widget, idx=index: asyncio.create_task(
    ...
    async def _on_process_files(self, widget) -> None:
        ...
        try:
            ...
            # Update status
            successful = len(
                [r for r in results if r.get("processing_status") == "success"]
            )
            self.status_label.text = (
                f"Processed {successful}/{len(results)} files successfully"
            )
        ...
        if hasattr(self.app, "chat_manager") and self.app.chat_manager:
            ...
            if status == "success":
                ...
                await self.app.chat_manager.handle_file_processed(
                    file_path.name, f"success ({content_type})"
                )
            else:
                error_msg = result.get("error", "Unknown error")
                await self.app.chat_manager.handle_file_processed(
                    file_path.name, f"error: {error_msg}"
                )
