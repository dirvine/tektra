...
class MessageBubble:
    ...
    def _create_bubble(self) -> toga.Box:
        ...
        message_container = toga.Box(style=Pack(
            direction=ROW,
            margin=(5, 10),
        ...
        bubble_wrapper = toga.Box(style=Pack(
            direction=COLUMN,
...
class ChatPanel:
    ...
    def _create_panel(self) -> toga.Box:
        ...
        self.messages_container = toga.Box(style=Pack(
            direction=COLUMN,
            align_items="start"
        ))
    ...
    def _create_input_area(self) -> toga.Box:
        ...
        input_container = toga.Box(style=Pack(
            direction=ROW,
            margin_top=10,
            align_items="center"
        ))
...
How can I help you today?"""
    ...
    def set_processing_state(self, is_processing: bool) -> None:
        ...
        # Update UI state
        self.send_btn.enabled = not is_processing and bool(self.message_input.value.strip())
    ...
    def _scroll_to_bottom(self) -> None:
        ...
        # Note: Toga's ScrollContainer auto-scrolling behavior may vary
        # This is a placeholder for explicit scrolling if needed
        pass
    ...
    async def _show_file_upload_dialog(self) -> None:
        ...
        try:
            ...
            self.add_message("system", "📎 File upload feature ready! Click 'Choose Files' to select files for analysis.")
    ...
    async def _on_file_processed_for_chat(self, file_path, result) -> None:
        ...
        if status == "success":
            self.add_message("system", f"✅ Processed {file_path.name} ({content_type})")
            ...
            self.add_message("system", f"❌ Failed to process {file_path.name}: {error_msg}")
    ...
    async def _add_file_to_conversation_context(self, file_path, result) -> None:
        ...
        try:
            ...
            if content_type == "image":
                ...
                context_message = f"📷 Image '{file_path.name}' uploaded and ready for analysis.\n"
                context_message += f"Dimensions: {analysis.get('dimensions', 'unknown')}\n"
                context_message += f"File size: {self._format_file_size(file_path.stat().st_size)}"
