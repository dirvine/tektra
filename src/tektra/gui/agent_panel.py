...
class AgentCreationWizard:
    ...
    def _build_ui(self) -> toga.Box:
        ...
        self.container = toga.Box(
            style=Pack(direction=COLUMN, background_color="#f8f9fa", padding=20)
        )
        ...
        header = toga.Label(
            "Create New Agent",
            style=Pack(
                font_size=20, font_weight="bold", margin_bottom=20, color="#2c3e50"
            ),
        )
        ...
        self.step_indicator = toga.Label(
            ...
            style=Pack(font_size=14, margin_bottom=15, color="#7f8c8d"),
        )
        self.container.add(self.step_indicator)

        # Step container (content changes based on step)
        self.step_container = toga.Box(
            style=Pack(direction=COLUMN, min_height=300, margin_bottom=20)
        )
        self.container.add(self.step_container)

        # Navigation buttons
        self.navigation_container = toga.Box(
        ...
        self.next_button = toga.Button(
            ...
            style=Pack(background_color="#007bff", color="#ffffff", width=100),
        )
    ...
    def _show_method_selection(self):
        ...
        # Template option
        template_box = toga.Box(
            style=Pack(direction=COLUMN, background_color="#ffffff", padding=15)
        )
    ...
    def _show_natural_language_input(self):
        ...
        self.description_input = toga.MultilineTextInput(
            ...
            style=Pack(min_height=150, margin_bottom=15, font_size=12),
        )
        ...
        for example in examples:
            ex_label = toga.Label(
                example, style=Pack(font_size=11, color="#6c757d", margin_bottom=3)
            )
    ...
    def _show_template_selection(self):
        ...
        category_label = toga.Label(
            "Select a category:", style=Pack(font_size=14, margin_bottom=10)
        )
        ...
        template_label = toga.Label(
            "Choose a template:", style=Pack(font_size=14, margin_bottom=10)
        )
        self.step_container.add(template_label)

        # Template container with scroll
        self.template_list_container = toga.Box(style=Pack(direction=COLUMN))

        self.template_scroll = toga.ScrollContainer(
            content=self.template_list_container, style=Pack(height=200)
        )
    ...
    def _show_templates_for_category(self, category: str):
        ...
        for template in templates:
            ...
            icon_label = toga.Label(
                template.icon, style=Pack(font_size=24, margin_right=15)
            )
            card.add(icon_label)
            ...
            name_label = toga.Label(
                template.name, style=Pack(font_weight="bold", margin_bottom=3)
            )
            info_box.add(name_label)

            desc_label = toga.Label(
                template.description, style=Pack(font_size=11, color="#6c757d")
            )
            ...
            select_btn = toga.Button(
                ...
                style=Pack(width=80, font_size=12),
            )
            card.add(select_btn)
    ...
    def _show_template_customization(self):
        ...
        # Template info
        info_box = toga.Box(
            style=Pack(direction=ROW, margin_bottom=15, align_items="center")
        )

        icon_label = toga.Label(
            self.selected_template.icon, style=Pack(font_size=20, margin_right=10)
        )
        info_box.add(icon_label)

        name_label = toga.Label(
            self.selected_template.name, style=Pack(font_weight="bold")
        )
        info_box.add(name_label)
        ...
        params_label = toga.Label(
            ...
            style=Pack(font_size=14, margin_bottom=10, font_weight="bold"),
        )
        ...
        for param_name, param_info in self.selected_template.parameters.items():
            # Parameter container
            param_box = toga.Box(style=Pack(direction=COLUMN, margin_bottom=15))
            ...
            if param_info["type"] == "string":
                ...
                input_widget = toga.Selection(
                    items=param_info["values"], style=Pack(width=300)
                )
                # Set default
                ...
                input_widget = toga.Switch(
                    text="", value=param_info.get("default", False)
                )
            elif param_info["type"] == "integer":
                input_widget = toga.NumberInput(
                    value=param_info.get("default", 0), style=Pack(width=100)
                )
            else:
                # Default to text input
                input_widget = toga.TextInput(style=Pack(width=300))
    ...
    def _show_confirmation(self):
        ...
        # Description
        desc_box = toga.Box(
            style=Pack(background_color="#f8f9fa", padding=15, margin_bottom=20)
        )
        ...
        for step in steps:
            step_label = toga.Label(
                step, style=Pack(font_size=12, margin_bottom=5, color="#495057")
            )
            ...
            if self.creation_method == "natural_language":
                ...
                if self.agent_builder:
                    ...
                    spec = AgentSpecification(
                        name="Custom Agent", description=self.agent_description
                    )
            else:
                ...
                spec = self.template_library.create_agent_from_template(
                    self.selected_template.name, customizations
                )
...
class AgentDashboard:
    ...
    def _build_ui(self) -> toga.Box:
        ...
        self.container = toga.Box(style=Pack(direction=ROW, background_color="#f8f9fa"))

        # Left panel - Agent list
        left_panel = toga.Box(
            style=Pack(
                direction=COLUMN, width=300, padding=10, background_color="#ffffff"
            )
        )
        ...
        # Agent list
        self.agent_list_container = toga.Box(style=Pack(direction=COLUMN))

        self.agent_scroll = toga.ScrollContainer(
            content=self.agent_list_container, style=Pack(flex=1)
        )
        ...
        self.detail_container = toga.Box(
            style=Pack(direction=COLUMN, flex=1, padding=20, background_color="#ffffff")
        )

        # Placeholder when no agent selected
        placeholder = toga.Label(
            "Select an agent to view details", style=Pack(font_size=14, color="#6c757d")
        )
    ...
    def _add_agent_card(self, agent_record):
        ...
        top_row = toga.Box(
            style=Pack(direction=ROW, align_items="center", margin_bottom=5)
        )
        ...
        status_label = toga.Label(
            status_color, style=Pack(font_size=12, margin_right=5)
        )
        top_row.add(status_label)

        # Name
        name_label = toga.Label(spec.name, style=Pack(font_weight="bold", flex=1))
        ...
        desc_label = toga.Label(
            ...
            style=Pack(font_size=11, color="#6c757d", margin_bottom=5),
        )
        card.add(desc_label)

        # Metrics row
        metrics_row = toga.Box(style=Pack(direction=ROW, align_items="center"))

        # Execution count
        exec_label = toga.Label(
            f"🔄 {agent_record.metrics.total_executions}",
            style=Pack(font_size=10, margin_right=10),
        )
        ...
        success_rate = (
            ...
            / agent_record.metrics.total_executions
            * 100
            if agent_record.metrics.total_executions > 0
            else 0
        )
        success_label = toga.Label(f"✓ {success_rate:.0f}%", style=Pack(font_size=10))
        ...
        view_btn = toga.Button(
            "View Details",
            on_press=lambda w, ar=agent_record: asyncio.create_task(
                self._show_agent_details(ar)
            ),
            style=Pack(width=100, font_size=11, margin_top=5),
        )
        card.add(view_btn)
    ...
    async def _show_agent_details(self, agent_record):
        ...
        # Header
        header_box = toga.Box(
            style=Pack(direction=ROW, align_items="center", margin_bottom=20)
        )

        name_label = toga.Label(
            spec.name, style=Pack(font_size=24, font_weight="bold", flex=1)
        )
        ...
        if self.runtime:
            if agent_record.status == AgentStatus.INACTIVE:
                start_btn = toga.Button(
                    ...
                    style=Pack(
                        background_color="#28a745", color="#ffffff", margin_right=10
                    ),
                )
                ...
                stop_btn = toga.Button(
                    ...
                    style=Pack(
                        background_color="#dc3545", color="#ffffff", margin_right=10
                    ),
                )
        ...
        metrics_header = toga.Label(
            "Performance Metrics",
            style=Pack(
                font_size=16, font_weight="bold", margin_top=20, margin_bottom=10
            ),
        )
        ...
        metric_items = [
            ...
            (
                "Success Rate",
                (
                    f"{metrics.successful_executions / metrics.total_executions * 100:.1f}%"
            ...
            (
                "Last Execution",
                (
                    metrics.last_execution_time.strftime("%Y-%m-%d %H:%M")
    ...
    def _add_info_row(self, label: str, value: str):
        ...
        label_widget = toga.Label(
            f"{label}:", style=Pack(width=120, font_weight="bold", font_size=12)
        )
        row.add(label_widget)

        value_widget = toga.Label(value, style=Pack(flex=1, font_size=12))
    ...
    def _add_execution_row(self, execution: dict[str, Any]):
        ...
        icon_label = toga.Label(status_icon, style=Pack(margin_right=10))
        row.add(icon_label)

        # Time
        time_label = toga.Label(
            datetime.fromisoformat(execution["started_at"]).strftime("%m/%d %H:%M"),
            style=Pack(font_size=11, width=100),
        )
        row.add(time_label)
        ...
        result_label = toga.Label(
            result_text, style=Pack(flex=1, font_size=11, color="#6c757d")
        )
    ...
    async def _stop_agent(self, agent_id: str):
        ...
        if self.runtime and self.registry:
            try:
                ...
                # Update status
                await self.registry.update_agent(agent_id, status=AgentStatus.INACTIVE)
    ...
    async def _on_search_change(self, widget):
        ...
        search_query = widget.value.lower().strip()
        
        try:
            # If search is empty, reload all agents
            if not search_query:
                await self._load_agents()
                return
            ...
            if self.registry:
                # Get all agents and filter by search query
                all_agents = await self.registry.list_agents()
                filtered_agents = []
                
                for agent_record in all_agents:
                    spec = agent_record.specification
                    # Search in name, description, and goal
                    search_fields = [
                        spec.name.lower(),
                        spec.description.lower(),
                        spec.goal.lower(),
                        spec.type.value.lower()
                    ]
                    
                    # Check if query matches any field
                    if any(search_query in field for field in search_fields):
                        filtered_agents.append(agent_record)
                
                # Clear existing agent cards
                for child in list(self.agent_list_container.children):
                    self.agent_list_container.remove(child)
                
                # Add filtered agent cards
                for agent_record in filtered_agents:
                    self._add_agent_card(agent_record)
                
                logger.debug(f"Filtered agents: {len(filtered_agents)} matches for '{search_query}'")
            
        except Exception as e:
            logger.error(f"Error during agent search filtering: {e}")
...
class AgentPanel:
    ...
    def _build_ui(self) -> toga.Box:
        ...
        # Tab selector
        self.tab_selector = toga.Box(
            style=Pack(
                direction=ROW, margin_bottom=10, background_color="#e9ecef", padding=5
            )
        )
        ...
        self.dashboard_tab = toga.Button(
            ...
            style=Pack(margin_right=10, background_color="#007bff", color="#ffffff"),
        )
    ...
    def _show_tab(self, tab_name: str):
        ...
        if tab_name == "dashboard":
            ...
            if not self.dashboard:
                self.dashboard = AgentDashboard(self.agent_registry, self.agent_runtime)
