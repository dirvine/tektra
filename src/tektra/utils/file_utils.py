...
class FileUtils:
    ...
    def validate_file(self, file_path: str | Path) -> dict[str, Any]:
        ...
        validation_result["mime_type"] = (
            mimetypes.guess_type(str(file_path))[0] or "unknown"
        )
        ...
        if validation_result["extension"] in self.SUPPORTED_IMAGE_FORMATS:
            ...
            if validation_result["size"] > self.MAX_IMAGE_SIZE:
                validation_result["errors"].append(
                    f"Image file too large (max {self.MAX_IMAGE_SIZE // (1024*1024)}MB)"
                )
            ...
            if validation_result["size"] > self.MAX_DOCUMENT_SIZE:
                validation_result["errors"].append(
                    f"Document file too large (max {self.MAX_DOCUMENT_SIZE // (1024*1024)}MB)"
                )
            ...
            if validation_result["size"] > self.MAX_AUDIO_SIZE:
                validation_result["errors"].append(
                    f"Audio file too large (max {self.MAX_AUDIO_SIZE // (1024*1024)}MB)"
                )
            ...
            if validation_result["size"] > self.MAX_VIDEO_SIZE:
                validation_result["errors"].append(
                    f"Video file too large (max {self.MAX_VIDEO_SIZE // (1024*1024)}MB)"
                )
        else:
            validation_result["errors"].append(
                f"Unsupported file type: {validation_result['extension']}"
            )
    ...
    def get_file_info(self, file_path: str | Path) -> dict[str, Any]:
        ...
        return {
            ...
            "hash_sha256": (
                self.get_file_hash(file_path) if file_path.is_file() else None
            ),
        }
