...
class DockerUtils:
    ...
    def is_container_running(self, container_name: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Container {container_name} not found or error: {e}")
            return False
    ...
    async def wait_for_service_health(
        ...
        for attempt in range(max_attempts):
            ...
            logger.debug(
                f"Service {service_url} not ready, attempt {attempt + 1}/{max_attempts}"
            )
            await asyncio.sleep(delay)

        logger.error(
            f"Service {service_url} failed to become healthy after {max_attempts} attempts"
        )
        return False
        ...
        try:
            ...
            if filter_labels:
                containers = [
                    c
                    for c in containers
                    if all(c.labels.get(k) == v for k, v in filter_labels.items())
                ]
        ...
        try:
            ...
            for container in containers:
                try:
                    ...
                    logger.debug(
                        f"Error getting stats for container {container.name}: {e}"
                    )
