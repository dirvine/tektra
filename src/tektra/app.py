...
from .ai.smart_router import SmartRouter, QueryContext, QueryRoute
from .ai.multimodal import MultimodalProcessor
from .voice import VoiceConversationPipeline
from .gui.chat_panel import ChatPanel, ChatManager
from .gui.main_window import MainWindow
from .gui.agent_panel import AgentPanel
from .utils.config import AppConfig
from .data.storage import DataStorage
from .agents import AgentBuilder, AgentRuntime, AgentRegistry, SandboxType
...
class TektraApp(toga.App):
    ...
    def build_header(self) -> toga.Box:
        ...
        self.model_status_indicator = toga.Label(
            "⚪ Models",
            style=Pack(margin_right=10, color="#95a5a6", font_size=12)
        )
    ...
    def build_right_sidebar(self) -> toga.Box:
        ...
        self.message_count_label = toga.Label(
            "Messages: 0",
            style=Pack(color="#95a5a6", font_size=11, margin_bottom=5)
        )
    ...
    async def initialize_backend_systems(self):
        ...
        try:
            ...
            qwen_config = QwenModelConfig(
                model_name=self.config.get('qwen_model_name', 'Qwen/Qwen2.5-VL-7B-Instruct'),
            ...
            self.show_error("Initialization Error", f"Failed to initialize backend systems:\n\n{e}")
    ...
    def update_conversation_stats(self):
        ...
        if self.smart_router:
            ...
            self.routing_stats_label.text = f"Unmute: {unmute_pct:.0f}% | Qwen: {qwen_pct:.0f}%"
    ...
    async def handle_chat_message(self, message: str):
        ...
        try:
            ...
            await self.chat_manager.process_user_message(
                message, 
                self.smart_router,
                context={"session_id": "main_session"}
            )
            
        except Exception as e:
            logger.error(f"Error handling chat message: {e}")
            if self.chat_panel:
                self.chat_panel.add_message("assistant", f"Sorry, I encountered an error: {e}")
        finally:
    ...
    async def start_voice_input(self, widget):
        ...
        # TODO: Implement push-to-talk functionality
        pass
    ...
    async def process_uploaded_file(self, file_path: str):
        ...
        try:
            ...
            if result['processing_status'] == 'success':
                ...
                if self.chat_manager:
                    await self.chat_manager.handle_file_processed(filename, f"error: {error}")
                
                if self.chat_panel:
                    self.chat_panel.add_message("assistant", f"Sorry, I couldn't process that file: {error}")
    ...
    async def show_settings(self, widget):
        ...
        self.main_window.info_dialog("Settings", "Settings dialog coming soon!")
    ...
    async def handle_agent_query(self, message: str) -> bool:
        ...
        for pattern in agent_patterns:
            if pattern in message_lower:
                ...
                self.chat_panel.add_message("assistant", "I'll help you create an agent. Let me analyze your requirements...")
                
                try:
                    # Create agent from description
                    spec = await self.agent_builder.create_agent_from_description(description)
...
You can manage your agents by clicking 'Manage Agents' in the sidebar."""
                    ...
                    self.chat_panel.add_message("assistant", f"I encountered an error creating the agent: {e}")
                
                return True
        
        # Check for agent management queries
        if any(phrase in message_lower for phrase in ["list agents", "show agents", "my agents"]):
            if self.agent_registry:
                agents = await self.agent_registry.list_agents()
                if agents:
                    response = "Here are your active agents:\n\n"
                    for agent in agents[:5]:  # Show first 5
                        response += f"• **{agent.specification.name}** - {agent.status.value}\n"
