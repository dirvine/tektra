...
def _serialize_agent_spec(spec: AgentSpecification) -> dict[str, Any]:
    ...
    if "type" in spec_dict:
        spec_dict["type"] = (
            spec_dict["type"].value
            if hasattr(spec_dict["type"], "value")
            else spec_dict["type"]
        )
...
class AgentRegistry:
    ...
    async def _init_database(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_agent_created ON agents(created_at)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_execution_agent ON agent_executions(agent_id)"
            )
    ...
    async def register_agent(self, spec: AgentSpecification) -> str:
        ...
        # Create agent record
        record = AgentRecord(
            specification=spec, status=AgentStatus.INACTIVE, metrics=AgentMetrics()
        )
    ...
    async def update_agent(
        self,
        agent_id: str,
        spec: AgentSpecification | None = None,
    ...
    async def list_agents(
        ...
        sorted_candidates = sorted(
            candidates, key=lambda aid: self.agents[aid].created_at, reverse=True
        )
    ...
    async def search_agents(self, query: str) -> list[AgentRecord]:
        ...
        for _agent_id, record in self.agents.items():
            ...
            if (
                ...
                or query_lower in spec.goal.lower()
            ):
    ...
    async def add_agent_relationship(
        self, parent_id: str, child_id: str, relationship_type: str = "parent-child"
    ):
        ...
        if metrics:
            if "memory_mb" in metrics:
                agent_metrics.peak_memory_mb = max(
                    agent_metrics.peak_memory_mb, metrics["memory_mb"]
                )
            if "cpu_percent" in metrics:
                ...
                agent_metrics.average_cpu_percent = (
                    prev_avg * (n - 1) + metrics["cpu_percent"]
                ) / n
    ...
    async def delete_agent(self, agent_id: str):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "DELETE FROM agent_history WHERE agent_id = ?", (agent_id,)
            )
            await db.execute(
    ...
    def _remove_from_indexes(self, agent_id: str, record: AgentRecord):
        ...
        for _index_type, index_dict in self.agent_index.items():
            for _key, agent_set in index_dict.items():
                ...
                async for row in cursor:
                    try:
                        ...
                        record = AgentRecord(
                            ...
                            collaborating_agents=relationships.get(
