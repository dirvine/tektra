...
from .builder import AgentSpecification, AgentType
from ..memory import TektraMemoryManager, MemoryConfig, MemoryType, MemoryContext
...
class AgentExecutionContext:
    ...
    # Memory integration
    memory_manager: Optional[TektraMemoryManager] = None
    session_id: str = None
...
class AgentSandbox:
    ...
    def __init__(self, sandbox_type: SandboxType = SandboxType.DOCKER):
        ...
        if sandbox_type == SandboxType.DOCKER:
            try:
                ...
                logger.warning(f"Docker not available: {e}, falling back to process isolation")
    ...
    async def execute_agent(
        ...
        """
        logger.info(f"Executing agent {context.agent_id} in {self.sandbox_type.value} sandbox")
    ...
    async def _execute_in_docker(self, context: AgentExecutionContext) -> Dict[str, Any]:
        ...
        container_config = {
            ...
            'volumes': {
                str(context.working_directory): {'bind': '/app', 'mode': 'rw'}
            },
    ...
    async def _execute_in_process(self, context: AgentExecutionContext) -> Dict[str, Any]:
    ...
    async def _execute_local(self, context: AgentExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                agent_module.run_agent(execution_input),
                timeout=context.timeout
            )
...
"""
    ...
    async def _write_runner_script(self, script_path: Path, context: AgentExecutionContext):
...
"""
    ...
    def _create_error_result(self, context: AgentExecutionContext, error: str) -> Dict[str, Any]:
...
class AgentRuntime:
    ...
    async def _execute_agent(self, context: AgentExecutionContext):
        ...
        try:
            ...
            json_safe_input = {k: v for k, v in execution_input.items() if k != 'memory_manager'}
    ...
    async def _load_agent_memory_context(self, context: AgentExecutionContext, user_id: Optional[str] = None):
        ...
        try:
            ...
            agent_memories = await context.memory_manager.get_agent_context(context.agent_id)
            ...
            if context.spec.persistent_memory:
                ...
                logger.info(f"Loaded {len(search_result.entries)} memory entries for agent {context.agent_id}")
    ...
    async def _save_execution_to_memory(self, context: AgentExecutionContext, result: Dict[str, Any]):
        ...
        try:
            ...
            result_summary = json.dumps(result.get('output', {}))[:500] if result.get('output') else "No output"
            ...
            await context.memory_manager.add_task_result(
                ...
                agent_id=context.agent_id,
                user_id=context.input_data.get('user_id') if isinstance(context.input_data, dict) else None
            )
    ...
    async def share_agent_memory(self, source_agent_id: str, target_agent_id: str, memory_types: List[MemoryType]):
            ...
            for memory in memories_to_share.entries:
                ...
                if memory.type == MemoryType.AGENT_CONTEXT:
                    await self.memory_manager.add_agent_context(
                        ...
                        importance=memory.importance * 0.8  # Slightly reduce importance for shared memories
                    )
                    shared_count += 1
            
            logger.info(f"Shared {shared_count} memories from {source_agent_id} to {target_agent_id}")
