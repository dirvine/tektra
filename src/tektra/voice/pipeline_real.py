...
class VoiceConversationPipeline:
    ...
    def __init__(
        ...
        # Initialize Unmute components with configuration
        self.service_manager = UnmuteServiceManager(unmute_path, config=service_config)
        self.unmute_client = UnmuteWebSocketClient(config=service_config)
            ...
            while self.is_recording and self.audio_stream:
                try:
                    ...
                    audio_data = self.audio_stream.read(
                        self.audio_config["chunk_size"], exception_on_overflow=False
                    )
    ...
    async def _play_audio(self, audio_data: bytes, format_info: dict[str, Any]):
        ...
        try:
            ...
            # For now, we rely on the Unmute TTS service to handle playback
            # In a full implementation, you could add:
            # 1. Direct audio playback using pyaudio or similar
            # 2. Audio queuing for smooth playback
            # 3. Volume control and audio effects
            
            # Basic logging to track audio flow
            if self.on_audio_response:
                await self._safe_callback(self.on_audio_response, audio_data)
            
            logger.debug("Audio playback delegated to Unmute TTS service")
