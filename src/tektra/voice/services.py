...
class UnmuteServiceManager:
    ...
    def __init__(self, unmute_path: Path, base_url: str = "http://localhost"):
        """
        ...
        self.service_ports = {
            ...
            "tts": 8002, 
            "llm": 8000,
            "backend": 8000  # Main backend service
        }
        ...
        if not self.compose_file.exists():
            logger.warning(f"Unmute docker-compose.yml not found at {self.compose_file}")
    ...
    async def wait_for_services(self, timeout: int = 120) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            logger.info(f"Healthy services: {len(healthy_services)}/{total_services} "
        ...
        unhealthy_services = [name for name, status in final_health.items() if not status]
        
        if unhealthy_services:
            logger.error(f"Services not ready after {timeout}s: {', '.join(unhealthy_services)}")
