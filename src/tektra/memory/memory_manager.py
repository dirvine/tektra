...
from .memory_types import (
    MemoryEntry, MemoryContext, MemorySearchResult, MemoryStats, MemoryType,
    create_conversation_memory, create_agent_context_memory, create_task_result_memory
)
...
class TektraMemoryManager:
    ...
    def __init__(self, config: Optional[MemoryConfig] = None):
        ...
        logger.info(f"Memory manager initialized with config: {self.config.storage_type}")
    ...
    async def search_memories(self, context: MemoryContext) -> MemorySearchResult:
        ...
        logger.debug(f"Memory search completed: {len(entries)} results in {search_time:.2f}ms")
        return result
