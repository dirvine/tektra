...
class DocumentProcessor:
    ...
    @staticmethod
    async def process_file(file_path: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if extension in DocumentProcessor.SUPPORTED_TEXT_FORMATS:
                result["content"] = await DocumentProcessor._process_text_file(file_path)
                ...
                result["content"] = await DocumentProcessor._process_docx_file(file_path)
            else:
...
class ImageProcessor:
    ...
    @staticmethod
    async def process_image(image_input: Union[str, Path, bytes, Image.Image]) -> Dict[str, Any]:
        """
    ...
    def _resize_image(image: Image.Image) -> Image.Image:
        ...
        if image.size[0] <= ImageProcessor.MAX_IMAGE_SIZE[0] and image.size[1] <= ImageProcessor.MAX_IMAGE_SIZE[1]:
            return image
    ...
    @staticmethod
    def _get_dominant_colors(img_array: np.ndarray, n_colors: int = 3) -> List[List[int]]:
...
class MultimodalProcessor:
    ...
    def __init__(self):
        ...
        logger.info(f"Multimodal processor initialized with {len(self.supported_formats)} supported formats")
    ...
    async def process_file(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            if extension in DocumentProcessor.SUPPORTED_TEXT_FORMATS | DocumentProcessor.SUPPORTED_DOCUMENT_FORMATS:
    ...
    async def prepare_for_ai_analysis(self, content: Dict[str, Any], query: str) -> Dict[str, Any]:
        """
    ...
    async def _prepare_document_analysis(self, content: Dict[str, Any], query: str) -> Dict[str, Any]:
...
Please provide a comprehensive analysis addressing the user's question."""
    ...
    async def _prepare_image_analysis(self, content: Dict[str, Any], query: str) -> Dict[str, Any]:
        """Prepare image content for AI vision analysis."""
        if content["processing_status"] != "success":
            raise ValueError(f"Cannot analyze failed image processing: {content.get('error')}")
...
Please provide a detailed analysis of the image addressing the user's question."""
    ...
    def get_processor_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.processed_files / max(1, self.processed_files + self.processing_errors),
            "supported_formats": list(self.supported_formats),
