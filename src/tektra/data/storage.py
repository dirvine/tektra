...
class DataStorage:
    ...
    def _get_default_data_dir(self) -> Path:
        ...
        if platform.system() == "Windows":
            ...
            data_dir = Path.home() / "Library" / "Application Support" / "Tektra" / "Data"
    ...
    async def get_conversation_history(
        ...
        return await self.conversation_storage.get_conversation_history(session_id, limit)
    ...
    async def perform_maintenance(self) -> Dict[str, int]:
        ...
        results['conversations_deleted'] = await self.conversation_storage.clear_old_conversations()
    ...
    async def export_data(self, export_path: str) -> bool:
        ...
        try:
            ...
            for session_id in self.session_storage.sessions:
                conversations = await self.get_conversation_history(session_id, limit=1000)
                export_data['conversations'][session_id] = conversations
                export_data['sessions'][session_id] = self.session_storage.sessions[session_id]
