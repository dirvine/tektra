...
class VectorDatabase:
    ...
    def init_embedding_model(self):
        ...
        try:
            ...
            logger.info(f"Embedding model '{self.embedding_model_name}' loaded (dimension: {self.dimension})")
    ...
    def init_vector_index(self):
        ...
        try:
            ...
            logger.info(f"Vector index initialized with {self.vector_index.ntotal} vectors")
    ...
    def search_similar(self, 
        ...
        try:
            ...
            logger.debug(f"Found {len(results)} similar texts for query: {query[:50]}...")
            return results
    ...
    def delete_by_source(self, source_type: str, source_id: str) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(f"Deleted {deleted_count} texts from source {source_type}:{source_id}")
    ...
    def get_id_by_hash(self, content_hash: str) -> Optional[str]:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if contents:
                    ...
                    if vectors is not None:
                        ...
                        conn.commit()
                        logger.info(f"Rebuilt vector index with {len(contents)} vectors")
