...
async def chat(
    message: ChatMessage, db: AsyncSession = Depends(get_db)
) -> ChatResponse:
    ...
    try:
        ...
        if message.stream:
            ...
            response = await ai_manager.chat(
                messages=ai_messages, model_name=message.model, stream=False
            )
        ...
        async def generate_stream():
            try:
                ...
                async for token in ai_manager.chat(
                    messages=ai_messages, model_name=message.model, stream=True
                ):
