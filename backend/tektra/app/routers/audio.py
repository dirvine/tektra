...
class TTSRequest(BaseModel):
    ...
    rate: Optional[str] = Field(
        None, description="Speaking rate (e.g., '+20%', '-10%')"
    )
    pitch: Optional[str] = Field(
        None, description="Voice pitch (e.g., '+5Hz', '-10Hz')"
    )
    volume: Optional[str] = Field(
        None, description="Voice volume (e.g., '+20%', '-10%')"
    )
...
async def transcribe_audio(
    ...
    try:
        ...
        try:
            if phi4_service.is_loaded:
                result = await phi4_service.transcribe_audio(
                    audio_data=audio_data, language=language, task=task
                )
            else:
                raise RuntimeError("Phi-4 not available")
        except Exception as phi4_error:
            logger.warning(
                f"Phi-4 transcription failed, using Whisper fallback: {phi4_error}"
            )
        ...
        logger.info(
            f"Transcribed audio for user {current_user.id}: {len(result['text'])} characters"
        )
...
async def detect_language(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Phi-4 language detection failed, using Whisper fallback: {phi4_error}"
            )
            result = await whisper_service.detect_language(audio_data)

        logger.info(
            f"Detected language for user {current_user.id}: {result['detected_language']}"
        )
...
async def synthesize_speech(
    ...
    try:
        ...
        content_type_map = {"mp3": "audio/mpeg", "wav": "audio/wav", "ogg": "audio/ogg"}
        content_type = content_type_map.get(request.output_format, "audio/mpeg")

        logger.info(
            f"Synthesized speech for user {current_user.id}: {len(request.text)} chars -> {len(audio_data)} bytes"
        )
...
async def synthesize_ssml(
    request: SSMLRequest, current_user: User = Depends(get_current_user)
):
    """
...
async def synthesize_speech_streaming(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Streaming synthesis failed: {str(e)}"
        )
...
async def get_audio_service_info(current_user: User = Depends(get_current_user)):
    """
...
async def unload_whisper_model(current_user: User = Depends(get_current_user)):
    """
...
async def load_phi4_model(current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if success:
            return {
                "message": "Phi-4 Multimodal model loaded successfully",
                "status": "loaded",
            }
        else:
...
async def unload_phi4_model(current_user: User = Depends(get_current_user)):
    """
...
async def get_phi4_info(current_user: User = Depends(get_current_user)):
    """
...
async def detect_voice_activity(
    ...
    try:
        ...
        logger.info(
            f"VAD completed for user {current_user.id}: {result['speech_ratio']:.2f} speech ratio"
        )
...
async def preprocess_audio(
    ...
    try:
        ...
        processed_audio, sample_rate = await vad_service.preprocess_audio(
            audio_data, reduce_noise=reduce_noise, normalize=normalize
        )
        ...
        logger.info(
            f"Audio preprocessed for user {current_user.id}: {len(audio_data)} -> {len(processed_data)} bytes"
        )
...
async def get_voice_segments(
    ...
    min_segment_duration: float = Query(
        0.5, description="Minimum segment duration in seconds"
    ),
    max_silence_gap: float = Query(
        0.3, description="Maximum silence gap to bridge segments"
    ),
    current_user: User = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        logger.info(
            f"Voice segmentation completed for user {current_user.id}: {len(segments)} segments found"
        )
...
async def check_silence(
    ...
    threshold_duration: float = Query(
        0.5, description="Minimum duration to consider for silence"
    ),
    current_user: User = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        # Check for silence
        is_silent = await vad_service.is_silence(
            audio_data, threshold_duration=threshold_duration
        )

        logger.info(
            f"Silence check for user {current_user.id}: {'silent' if is_silent else 'contains audio'}"
        )
...
async def configure_vad(
    ...
    try:
        ...
        if not 0 <= aggressiveness <= 3:
            raise HTTPException(
                status_code=400, detail="Aggressiveness must be between 0 and 3"
            )
        ...
        logger.info(
            f"VAD configured by user {current_user.id}: aggressiveness={aggressiveness}"
        )
...
async def get_supported_languages(
    ...
    try:
        ...
        logger.info(
            f"Languages requested by user {current_user.id}: {len(languages)} languages"
        )
...
async def detect_language(
    file: Optional[UploadFile] = File(
        None, description="Audio file for language detection"
    ),
    text: Optional[str] = Form(None, description="Text for language detection"),
    prefer_audio: bool = Form(
        True, description="Prefer audio detection when both available"
    ),
    current_user: User = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        if not audio_data and not text:
            raise HTTPException(
                status_code=400, detail="Either audio file or text must be provided"
            )

        # Detect language
        result = await detect_language_auto(
            audio_data=audio_data, text=text, prefer_audio=prefer_audio
        )

        logger.info(
            f"Language detection for user {current_user.id}: {result['detected_language']} ({result['confidence']:.2f})"
        )
...
async def get_voices_for_language(
    language: str, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Voices for {language} requested by user {current_user.id}: {len(voices)} voices"
        )
        ...
        return {
            ...
            "default_voice": (
                language_info.get("default_voice") if language_info else None
            ),
        }
...
async def recommend_voice_for_language(
    ...
    try:
        ...
        if not recommended_voice:
            raise HTTPException(
                status_code=404, detail=f"No voices available for language '{language}'"
            )
        ...
        logger.info(
            f"Voice recommendation for user {current_user.id}: {language} -> {recommended_voice}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Voice recommendation failed: {str(e)}"
        )
...
async def auto_configure_voice(
    ...
    try:
        ...
        configuration = await configure_voice_for_content(
            audio_data=audio_data, text=text, user_preferences=user_preferences
        )

        logger.info(
            f"Auto voice configuration for user {current_user.id}: {configuration['language']} -> {configuration['voice_id']}"
        )
...
async def get_language_service_info(current_user: User = Depends(get_current_user)):
    """
