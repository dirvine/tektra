...
async def handle_websocket_message(
    session_id: str, message: WebSocketMessage, user: User
):
    ...
    try:
        ...
        async with get_db_session() as db:
            if chat_msg.conversation_id:
                ...
                conversation = Conversation(
                    ...
                    title=(
                        chat_msg.content[:50] + "..."
                        if len(chat_msg.content) > 50
                        else chat_msg.content
                    ),
                    category="general",
                )
                db.add(conversation)
        ...
        await manager.send_message(
            session_id,
            {
                ...
                "data": {"conversation_id": conversation.id, "finished": True},
            },
        )
        ...
        await manager.send_message(
            session_id,
            {"type": "error", "data": {"message": f"Chat processing failed: {str(e)}"}},
        )
...
async def handle_audio_chunk(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        should_transcribe = (
            not session_info.get("vad_enabled", True)  # VAD disabled
            or (vad_result and vad_result["has_voice"])  # Voice detected
            or len(audio_bytes) > 2048  # Large chunk (likely contains speech)
        )
                    ...
                    await manager.send_message(
                        session_id,
                        {
                            ...
                            "data": {
                                ...
                                "vad_confidence": (
                                    vad_result["confidence"] if vad_result else 1.0
                                ),
                            },
                        },
                    )
...
async def process_complete_audio(session_id: str, user: User):
    ...
    try:
        ...
        if session_info.get("auto_language_detection", True):
            try:
                lang_detection = await language_service.detect_language_from_audio(
                    processed_combined
                )
                ...
                if lang_detection["is_reliable"]:
                    ...
                    await manager.send_message(
                        session_id,
                        {
                            ...
                            "data": {
                                ...
                                "language_name": lang_detection.get(
                                    "language_name", detected_lang
                                ),
                                "native_name": lang_detection.get(
                                    "native_name", detected_lang
                                ),
                            },
                        },
                    )
                    ...
                    if session_info.get("voice_auto_config", True):
                        voice_config = await language_service.auto_configure_voice(
                            ...
                            {
                                "gender": getattr(
                                    session_info["voice_config"], "gender", None
                                ),
                                "voice_type": "Neural",
                            },
                        )
                        ...
                        await manager.send_message(
                            session_id,
                            {"type": "voice_auto_configured", "data": voice_config},
                        )
        ...
        try:
            ...
            logger.warning(
                f"Phi-4 final transcription failed, using Whisper: {phi4_error}"
            )
            result = await whisper_service.transcribe_audio(
                audio_data=processed_combined,
                language=target_language,
...
async def handle_transcription_request(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        await manager.send_message(
            session_id, {"type": "transcription_result", "data": result}
        )
        ...
        await manager.send_message(
            session_id,
            {"type": "error", "data": {"message": f"Transcription failed: {str(e)}"}},
        )
...
async def handle_tts_request(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        await manager.send_message(
            session_id,
            {"type": "error", "data": {"message": f"TTS generation failed: {str(e)}"}},
        )
...
async def handle_vad_config(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        if "noise_reduction_enabled" in data:
            session_info["noise_reduction_enabled"] = bool(
                data["noise_reduction_enabled"]
            )
            ...
            session_id,
            {
                ...
                "data": {
                    ...
                    "settings": {
                        ...
                        "noise_reduction_enabled": session_info[
...
async def handle_language_detection(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        await manager.send_message(
            session_id, {"type": "language_detection_result", "data": result}
        )
...
async def handle_language_config(session_id: str, data: Dict, user: User):
    ...
    try:
        ...
        if "auto_language_detection" in data:
            session_info["auto_language_detection"] = bool(
                data["auto_language_detection"]
            )
            ...
            session_id,
            {
                ...
                "data": {
                    ...
                    "settings": {
                        ...
                        "auto_language_detection": session_info[
                            "auto_language_detection"
                        ],
                        "voice_auto_config": session_info["voice_auto_config"],
                        "language_confidence_threshold": session_info[
...
def get_connection_stats() -> Dict[str, Any]:
    ...
    return {
        ...
        "audio_sessions": len(
            [s for s in manager.audio_sessions.values() if s.get("is_speaking")]
        ),
        "session_details": [
            {
...
async def broadcast_message(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    await broadcast_system_message(
        message.get("message", ""), message.get("type", "system")
    )
    return {
        "status": "success",
...
async def send_user_message(user_id: int, message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if user_id in manager.user_sessions:
        await send_user_notification(
            user_id, message.get("message", ""), message.get("type", "info")
        )
        return {"status": "success", "message": f"Message sent to user {user_id}"}
    else:
        return {"status": "error", "message": f"User {user_id} not connected"}
