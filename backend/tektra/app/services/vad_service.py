...
class VADService:
    ...
    def _initialize_vad(self):
        ...
        try:
            ...
            logger.info(
                f"WebRTC VAD initialized with aggressiveness {self.aggressiveness}"
            )
        ...
        try:
            ...
            if input_sample_rate != self.sample_rate:
                audio_array = librosa.resample(
                    audio_array, orig_sr=input_sample_rate, target_sr=self.sample_rate
                )
            ...
            logger.info(
                f"Preprocessed audio: {len(audio_array)} samples at {self.sample_rate}Hz"
            )
            return audio_array, self.sample_rate
    ...
    async def detect_voice_activity(
        ...
        try:
            ...
            logger.debug(
                f"VAD result: {speech_ratio:.2f} speech ratio, {has_voice} voice detected"
            )
            return result
    ...
    async def _energy_based_vad(
        ...
        try:
            ...
            logger.debug(
                f"Energy-based VAD: {speech_ratio:.2f} speech ratio, {has_voice} voice detected"
            )
            return result
    ...
    def _find_speech_segments(self, voice_frames: List[bool]) -> List[Dict]:
        ...
        if (
            ...
            and current_segment["frames"] >= self.min_speech_frames
        ):
            segments.append(current_segment)
    ...
    async def is_silence(
        ...
        try:
            ...
            is_silent = (
                not vad_result["has_voice"]
                or vad_result["speech_ratio"] < 0.1
                or vad_result["duration"] < threshold_duration
            )
...
async def detect_voice_in_audio(
    audio_data: Union[bytes, np.ndarray], sample_rate: Optional[int] = None
) -> bool:
    """
