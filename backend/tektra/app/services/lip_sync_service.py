...
try:
    ...
    librosa = None
    logging.info(
        "librosa not available - using basic lip-sync analysis (install with: pip install librosa)"
    )
...
class LipSyncAnalyzer:
    ...
    def _estimate_phonemes_from_audio(
        ...
        try:
            ...
            times = librosa.frames_to_time(
                np.arange(len(energy)), sr=self.sample_rate, hop_length=self.hop_length
            )
            ...
            for i, (time, eng) in enumerate(zip(times, energy)):
                if eng > 0.01:  # Voice activity threshold
                    ...
                    if i < len(mfccs[0]):
                        ...
                        phoneme = self._classify_phoneme_from_features(
                            spectral_centroid, eng
                        )
            ...
            logger.warning(
                f"Advanced phoneme estimation failed: {e}, using basic method"
            )
            return self._basic_phoneme_estimation(audio_data)

    def _basic_phoneme_estimation(
        self, audio_data: np.ndarray
    ) -> List[Tuple[str, float, float]]:
    ...
    def _classify_phoneme_from_features(
        self, spectral_centroid: float, energy: float
    ) -> str:
                ...
                keyframes.append(
                    {
                        ...
                        "mouth_shape": viseme["viseme"],
                        "mouth_opening": viseme["intensity"]
                        * 0.7,  # Reduce intensity for transition
...
class LipSyncService:
    ...
    async def complete_speech_session(
        self, session_id: str, final_audio: bytes
    ) -> Dict[str, Any]:
