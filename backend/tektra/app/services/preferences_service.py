...
class PreferencesService:
    ...
    async def get_user_preferences(
        self, db: AsyncSession, user_id: int, create_if_missing: bool = True
    ) -> Optional[UserPreferences]:
        """Get user preferences, optionally creating defaults if missing."""
        try:
            preferences = (
                await db.query(UserPreferences)
                .filter(UserPreferences.user_id == user_id)
                .options(selectinload(UserPreferences.model_settings))
                .first()
            )
...
class ModelSettingsService:
    ...
    async def get_model_settings(
        self, db: AsyncSession, user_id: int, model_name: str
        ...
        try:
            ...
            if not settings:
                settings = ModelSettings(
                    user_preferences_id=preferences.id, model_name=model_name
                )
                db.add(settings)
    ...
    async def get_all_model_settings(
        ...
        try:
            ...
            settings = (
                await db.query(ModelSettings)
                .filter(ModelSettings.user_preferences_id == preferences.id)
                .all()
            )
...
class ConversationTemplateService:
    ...
    async def create_template(
        self, db: AsyncSession, user_id: int, template_data: Dict[str, Any]
    ) -> ConversationTemplate:
        """Create a new conversation template."""
        try:
            template = ConversationTemplate(user_id=user_id, **template_data)
    ...
    async def update_template(
        ...
        try:
            template = (
                await db.query(ConversationTemplate)
                .filter(
                    and_(
    ...
    async def delete_template(
        self, db: AsyncSession, template_id: int, user_id: int
    ) -> bool:
...
class APIKeyService:
    ...
    async def list_api_keys(self, db: AsyncSession, user_id: int) -> List[APIKey]:
        """List all API keys for a user (without decryption)."""
        try:
            api_keys = await db.query(APIKey).filter(APIKey.user_id == user_id).all()
